<!-- GENERATED by scripts/sync-agent-config.mjs — DO NOT EDIT.
     Edit the source rule, not this file: .cursor/rules/package-overview.mdc
     This is a projection of that rule, produced by scripts/sync-agent-config.mjs
     in the workspace umbrella repo (github.com/saputello2/keen-workspace), which
     is not part of this repository — so there is no in-repo command to
     regenerate or verify it, and no CI job that detects drift between the two.
